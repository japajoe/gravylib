...
namespace Gravy::System::IO
{
    ...
    size_t MemoryStream::Read(void *buffer, size_t numBytes)
    {
        ...
        std::memcpy(pBuffer, static_cast<char*>(data) + readPosition, numBytes);
        readPosition += numBytes;

        return numBytes;
    }

    size_t MemoryStream::Write(const void *buffer, size_t numBytes)
    {
        ...
        std::memcpy(static_cast<char*>(data) + writePosition, static_cast<const char *>(buffer), numBytes);
