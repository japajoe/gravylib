#include "Rfc2898DeriveBytes.hpp"
#include <string.h>
#include <stdio.h>
#include <stdlib.h>
...
static void PKCS5_PBKDF2_HMAC(unsigned char *password, size_t plen,
    ...
    unsigned char *output, sha2_context *context)
{
	sha2_context ctx;
...
namespace Gravy::System::Security::Cryptography
{
    ...
    std::vector<uint8_t> Rfc2898DeriveBytes::Pbkdf2(uint8_t *password, size_t passwordLength, uint8_t *salt, size_t saltLength, size_t iterations, size_t keyLength)
    {
        std::vector<uint8_t> key;
        key.resize(keyLength);
        memset(&key[0], 0, keyLength);
        PKCS5_PBKDF2_HMAC(password, passwordLength, salt, saltLength, iterations, keyLength, key.data(), &context);
        return key;
    }

    void Rfc2898DeriveBytes::Reset()
    {
        sha2_hmac_reset(&context);
    }
};